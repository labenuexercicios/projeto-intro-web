...
},
{
    ...
    estoque: false,
    valor: 5,
    ingredientes: [,"Óleo","Açúcar","Leite","Ovo"]
}
]
...
for (let i =0; i<listaProdutos.length; i++){
    console.log ("Os ingredientes para  " + listaProdutos[i].nome +  "  são: ")
    const itensProduto = listaProdutos[i].ingredientes
    //console.table(itensProduto)
    if (itensProduto.length){
    for (let k =0; k< itensProduto.length; k++){
        console.log(itensProduto[k])
    }
    }else {
    console.log("Item sem ingredientes cadastrados")
    }

}

// const recebeArrayObjetos = (arrayInformado,stringInformada)=>{
//     const resultado = arrayInformado.filter((objeto)=>{
//         return objeto.nome === stringInformada
//     })
...
const resultado = arrayInformado.filter((pedevela)=>{
    console.log(pedevela)
    console.log(pedevela.nome)
